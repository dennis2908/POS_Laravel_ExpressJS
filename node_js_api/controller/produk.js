...
exports.list_view = function(req, res){
  req.getConnection(function(err, connection){
    connection.query('select * from brg order by id DESC', function (error, results, fields) {
...
exports.del = function(req, res){
  var id = req.params.id;
   req.getConnection(function(err, connection){
    connection.query("DELETE FROM brg WHERE id = ? ", [id], function(err, rows){
		return res.send();
    });
  });
};
...
exports.get_all = function(req, res){
  var id = req.params.id;
   req.getConnection(function(err, connection){
    connection.query("select * from brg WHERE id = ? ", [id], function(err, rows){
...
exports.save = function(req, res){
  ...
  var query = connection.query("select * from brg WHERE id = "+input.id, function(err, rows){
  ...
  if(rows)
	  connection.query("UPDATE brg set ? WHERE id = ?", [data, input.id], function(err, row){
      if(err)
    ...
    req.getConnection(function(err, connection){
    var query = connection.query("INSERT INTO brg set ?", data, function(err, rows, fields){
        if(err)
